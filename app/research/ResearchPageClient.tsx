  ...
  const totalPlantsProduced = investigacionesTerminadas.reduce((total, subsection) => {
    return (
      ...
      subsection.projects.reduce((subTotal, project) => {
        if ('plantsProduced' in project && project.plantsProduced) {
          const number = Number.parseInt((project.plantsProduced as string).replace(/[^0-9]/g, ""))
      ...
      <section className="py-12 px-4 bg-white border-t border-gray-200">
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            <a href="/control-biologico-guide" className="group relative bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-6 border border-purple-200 hover:border-purple-500 hover:shadow-md transition-all duration-300 cursor-pointer">
              <Shield className="w-10 h-10 text-purple-600 mb-3" />
              <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-purple-700 transition-colors">Control Biológico</h3>
              <p className="text-gray-700 text-sm">Trichoderma, Trichogramma, Paratheresia claripalpis. Agentes microbianos para manejo sostenible de plagas</p>
              <div className="mt-3 text-purple-600 text-xs font-semibold group-hover:gap-1 flex items-center gap-0 transition-all opacity-0 group-hover:opacity-100">
                Ver más →
              </div>
            </a>

            <a href="/research?tab=overview" className="group relative bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-6 border border-blue-200 hover:border-blue-500 hover:shadow-md transition-all duration-300 cursor-pointer">
              <Microscope className="w-10 h-10 text-blue-600 mb-3" />
              <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-blue-700 transition-colors">Secuenciamiento</h3>
              <p className="text-gray-700 text-sm">Análisis genómico avanzado de patógenos y organismos beneficiosos</p>
              <div className="mt-3 text-blue-600 text-xs font-semibold group-hover:gap-1 flex items-center gap-0 transition-all opacity-0 group-hover:opacity-100">
                Ver más →
              </div>
            </a>

            <a href="/research?tab=overview" className="group relative bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-6 border border-green-200 hover:border-green-500 hover:shadow-md transition-all duration-300 cursor-pointer">
              <Dna className="w-10 h-10 text-green-600 mb-3" />
              <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-green-700 transition-colors">Mejoramiento Genético</h3>
              <p className="text-gray-700 text-sm">Desarrollo de variedades resistentes, productivas y de calidad superior</p>
              <div className="mt-3 text-green-600 text-xs font-semibold group-hover:gap-1 flex items-center gap-0 transition-all opacity-0 group-hover:opacity-100">
                Ver más →
              </div>
            </a>

            <a href="/research?tab=overview" className="group relative bg-gradient-to-br from-cyan-50 to-cyan-100 rounded-lg p-6 border border-cyan-200 hover:border-cyan-500 hover:shadow-md transition-all duration-300 cursor-pointer">
              <Leaf className="w-10 h-10 text-cyan-600 mb-3" />
              <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-cyan-700 transition-colors">Biotecnología Molecular</h3>
              <p className="text-gray-700 text-sm">Herramientas moleculares sostenibles para agricultura moderna</p>
              <div className="mt-3 text-cyan-600 text-xs font-semibold group-hover:gap-1 flex items-center gap-0 transition-all opacity-0 group-hover:opacity-100">
                Ver más →
              </div>
            </a>
          </div>
