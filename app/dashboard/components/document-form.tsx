            ...
            <div class="info-box">
              ...
              ${
                doc.pathogenicityTest?.diasObservacion && doc.pathogenicityTest.diasObservacion.length > 0
                  ? '<div style="margin-bottom: 10px; border-bottom: 1px solid #ddd; padding-bottom: 6px;"><div style="font-weight: bold; font-size: 9px; margin-bottom: 4px;">OBSERVACIONES DIARIAS</div>' + doc.pathogenicityTest.diasObservacion.map(obs => 
                      '<div style="margin-bottom: 6px; padding: 4px; background: #f9f9f9; border-left: 2px solid #999; font-size: 8px;"><strong>Día ' + obs.dia + ':</strong> Intensidad: ' + (obs.intensidad || "____") + ' | Planta: ' + (obs.porcentajePlanta || "____") + '%<br/>Síntomas: ' + (obs.sintomas || "____") + (obs.observaciones ? '<br/>Obs: ' + obs.observaciones : '') + '</div>'
                    ).join('') + '</div>'
                  : ''
              }
              ...
              ${
                doc.pathogenicityTest?.imagenes && doc.pathogenicityTest.imagenes.length > 0
                  ? '<div style="margin-bottom: 10px; border-bottom: 1px solid #ddd; padding-bottom: 6px;"><div style="font-weight: bold; font-size: 9px; margin-bottom: 4px;">IMÁGENES DE LA PRUEBA (' + doc.pathogenicityTest.imagenes.length + ')</div><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px;">' + doc.pathogenicityTest.imagenes.map(img =>
                        '<div style="border: 1px solid #ddd; padding: 4px; border-radius: 4px; text-align: center;"><img src="' + img.url + '" style="width: 100%; height: 100px; object-fit: contain; margin-bottom: 4px;" alt="Día ' + img.dia + '"/><div style="font-size: 8px; font-weight: bold;">Día ' + img.dia + '</div>' + (img.descripcion ? '<div style="font-size: 7px; color: #666; margin-top: 2px;">' + img.descripcion + '</div>' : '') + '</div>'
                      ).join('') + '</div></div>'
                  : ''
              }
              ...
              ${doc.pathogenicityTest?.conclusiones ? '<div style="margin-bottom: 10px;"><div style="font-weight: bold; font-size: 9px; margin-bottom: 4px;">CONCLUSIONES</div><div style="font-size: 8px; line-height: 1.4; padding: 4px; background: #fafafa; border-left: 2px solid #999;">' + doc.pathogenicityTest.conclusiones + '</div></div>' : ""}

              ${doc.pathogenicityTest?.notas ? '<div style="font-size: 8px; padding: 4px; background: #fafafa; border-left: 2px solid #999;"><strong>Notas:</strong> ' + doc.pathogenicityTest.notas + '</div>' : ""}
            </div>`
              : ""
          }
