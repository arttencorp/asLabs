    ...
    <div className="min-h-screen overflow-hidden bg-[#f4f6f1] font-[var(--font-poppins)] text-[#16352a]">
      ...
      <main>
        ...
        <section id="variedades" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-28">
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{varieties.map((item, index) => <motion.article key={item.code} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .07 }} className="group overflow-hidden rounded-[28px] border border-black/[.07] bg-white shadow-[0_16px_50px_rgba(15,55,45,.07)] transition duration-500 hover:-translate-y-2"><div className="relative h-[280px] overflow-hidden"><Image src={item.image} alt={`${item.type} ${item.name}`} fill className="object-cover transition duration-700 group-hover:scale-105" sizes="(min-width:1024px) 24vw,(min-width:640px) 48vw,100vw" /><div className="absolute inset-0 bg-gradient-to-t from-[#0b2b22]/80 via-transparent to-transparent" /><span className="absolute left-4 top-4 rounded-full border border-white/20 bg-black/20 px-3 py-2 text-[9px] font-bold uppercase tracking-[.15em] text-white backdrop-blur-xl">{item.code}</span><div className="absolute bottom-4 left-4 right-4 text-white"><p className="text-[9px] font-bold uppercase tracking-[.16em] text-[#ffc56f]">{item.type}</p><h3 className="mt-2 text-xl font-bold">{item.name}</h3></div></div><div className="flex items-center justify-between p-5"><span className="text-xs font-bold text-[#33705c]">Evaluar disponibilidad</span><ArrowRight className="h-4 w-4 text-[#33705c] transition group-hover:translate-x-1" /></div></motion.article>)}</div>
        </section>
