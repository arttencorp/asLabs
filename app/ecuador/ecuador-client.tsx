        ...
        <section data-navbar-theme="dark" className="relative min-h-[690px] overflow-hidden bg-[#092f20] text-white sm:min-h-[710px] lg:min-h-[730px]">
          ...
          <div className="relative mx-auto flex min-h-[690px] max-w-[1480px] items-center px-5 pb-24 pt-24 sm:min-h-[710px] sm:px-8 lg:min-h-[730px] lg:px-10">
            <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .75, delay: .1, ease: [.21, .47, .32, .98] }} className="max-w-[760px]">
              <div className="inline-flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-[.2em] text-[#c7e7b8]"><span className="grid h-7 w-7 place-items-center rounded-full border border-[#c7e7b8]/40">🇪🇨</span>Nueva sede · Quito</div>
              <h1 className="mt-5 max-w-[720px] text-balance text-[clamp(2.25rem,4.7vw,4.35rem)] font-medium leading-[1.01] tracking-[-.045em]">Ciencia especializada para el agro ecuatoriano</h1>
              <p className="mt-5 max-w-2xl text-sm leading-6 text-white/75 sm:text-base sm:leading-7">Biología molecular, formulaciones bacterianas y plantines in vitro con un enfoque preventivo frente a Fusarium.</p>
              ...
              <div className="mt-4 grid max-w-[690px] gap-2 sm:grid-cols-2">
                <div className="flex items-center gap-3 rounded-xl border border-white/20 bg-black/[.17] p-3 backdrop-blur-sm"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-[#f0a23a] text-[#173428]"><MapPin className="h-5 w-5" /></span><div><p className="text-[8px] font-bold uppercase tracking-[.14em] text-[#ffc56f]">Sede Ecuador</p><p className="mt-1 text-[11px] font-semibold leading-4">Edificio Carolina Millenium · Quito</p></div></div>
